...
2. **Database.** Apply `supabase/migrations/0015_stripe.sql`, then `0016_delivery_method.sql`, then `0017_checkout_address.sql`, then `0018_guest_orders.sql` (after `0014`), then `0027_house_prices.sql` (after `0026`), then `0028_house_price_30ml.sql`, then `0029_checkout_security.sql`.
3. **Webhook.** Dashboard → Developers → Webhooks → Add endpoint `https://<your site>/api/stripe/webhook` with events `checkout.session.completed`, `checkout.session.async_payment_succeeded`, `invoice.upcoming`, `invoice.paid`, `customer.subscription.deleted`, `checkout.session.expired`, `charge.refunded`, `charge.dispute.created`. Paste its signing secret into `STRIPE_WEBHOOK_SECRET`.
