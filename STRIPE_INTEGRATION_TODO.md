...
3. **Webhook.** Dashboard → Developers → Webhooks → Add endpoint `https://<your site>/api/stripe/webhook` with events `checkout.session.completed`, `checkout.session.async_payment_succeeded`, `invoice.upcoming`, `invoice.paid`, `customer.subscription.deleted`, `charge.refunded`, `charge.dispute.created`. Paste its signing secret into `STRIPE_WEBHOOK_SECRET`.
